...
class Cube():
    ...
    def isAffected(self, axis, layer, dir):
        return self.current_i[axis] == layer

    def update(self, axis, layer, dir):

        if not self.isAffected(axis, layer, dir):
            return
    ...
    def draw(self, col, surf, vert, animate, angle, axis, layer, dir):

        glPushMatrix()
        if animate and self.isAffected(axis, layer, dir):
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == KEYDOWN:
                    ...
                    if not animate and event.key in rot_layer_map:
                        animate, action = True, rot_layer_map[event.key]
                if event.type == KEYUP:
                    if event.key in rot_cube_map:
                        rot_cube = (0, 0)
